    ...
    def blue_option(self, frame):
        ...
        if self.is_open:
            if self.bluepy.is_connected():
                arduino_data = self.bluepy.read_bytes()
                print("reading  ", arduino_data)
                if arduino_data == 0:
                    self.reset_blue_start = True
                    self.is_open = False
                    self.put_label_data("UnKnown")
                    print("Message Read")
                    self.bluepy.reset_buffer()
        else:
            if not self.reset_blue_start:
                ...
                if _id != "UnKnown" and _id != "NoFace":
                    self.put_history_data(_id, vot, av, 'Bt')
                    self.put_label_data(_id)
                    if self.bluepy.is_connected():
    ...
    def put_history_data(self, _id, vot, av, bt_text):
        if _id != "UnKnown" and _id != "NoFace":
            self.history_data.append(
                (_id, DataBase.get_current_time(), DataBase.dic_to_text(vot, False), DataBase.dic_to_text(av, True), bt_text))
        elif _id == "UnKnown":
            self.history_data.append(
                ("UnKnown", DataBase.get_current_time(), DataBase.dic_to_text(vot, False), DataBase.dic_to_text(av, True), bt_text))
